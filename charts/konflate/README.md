...
| config.basePath | string | `""` | URL path prefix konflate is served under (e.g. /platform/konflate). Empty = root. The app registers every route under this prefix; configure your ingress to forward requests with the prefix preserved. |
...
| config.publicUrl | string | `""` | konflate's externally-reachable base URL (e.g. https://konflate.example.com); a posted status/comment links back to it. Empty = posted without a link. Should include `basePath` when serving under a subpath. |
